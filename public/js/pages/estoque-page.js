...
function subscribeRealtime() {
    supabase
        ...
        .on("postgres_changes", { event: "*", schema: "public", table: "tool" }, () => fetchTools())
        .subscribe();
}
function getFilteredAndSortedTools() {
    const filtered = tools.filter((tool) => {
        const s = searchTerm.toLowerCase();
        const matchesSearch = tool.name.toLowerCase().includes(s) ||
...
function renderQuickStats() {
    ...
    grid.append(stat("Ferramentas", String(totalTools)), stat("Estoque Baixo", String(lowStockTools)), stat("Total de Itens", String(totalQuantity)), stat("Média por Item", String(avgQuantity)));
    statsCard.append(title, grid);
    content.append(statsCard);
    function stat(label, value) {
        const c = el("div", { classes: ["stats-card"] });
        c.append(el("p", { text: label, attrs: { style: "margin:0;color:var(--muted);font-size:.9rem;" } }), el("h4", { text: value, attrs: { style: "margin:8px 0;font-size:2.2rem;color:var(--sec2);" } }));
        return c;
    }
}
function renderSearchAndFilters() {
    ...
    filterSelect.append(el("option", { text: "Todos os itens", attrs: { value: "all" } }), el("option", { text: "Estoque baixo", attrs: { value: "low" } }), el("option", { text: "Estoque OK", attrs: { value: "ok" } }));
    const sortSelect = el("select", {
        classes: ["filter-select"],
        attrs: { value: sortBy, onchange: "handleSort(this.value)" },
    });
    sortSelect.append(el("option", { text: "Ordenar por nome", attrs: { value: "name" } }), el("option", { text: "Ordenar por quantidade", attrs: { value: "quantity" } }), el("option", { text: "Ordenar por localização", attrs: { value: "location" } }));
    ...
    content.append(card);
    window.handleSearch = (v) => { searchTerm = v; renderToolsList(); };
    window.handleFilter = (v) => { filterStatus = v; renderToolsList(); };
    window.handleSort = (v) => { sortBy = v; renderToolsList(); };
}
function renderToolsList() {
    ...
    if (!filtered.length) {
        ...
        empty.append(el("h4", { text: "Nenhuma ferramenta encontrada", attrs: { style: "margin:0 0 10px 0;" } }), el("p", { text: "Ajuste os filtros de busca ou adicione uma nova ferramenta.", attrs: { style: "margin:0;" } }));
        card.append(empty);
    }
    else {
        const grid = el("div", {
...
function createToolCard(tool) {
    ...
    info.append(kv("Quantidade:", String(tool.quantity), low ? "var(--warning)" : "var(--success)"), kv("Mínimo:", String(tool.min_quantity)), kv("Local:", tool.location || "Não informado"));
    const actions = el("div", { attrs: { style: "display:flex;gap:8px;" } });
    const edit = el("button", { classes: ["btn", "btn-secondary", "btn-sm"], attrs: { onclick: `editTool('${tool.id}')` } });
    edit.innerHTML = '<i class="fas fa-edit"></i> Editar';
    const del = el("button", { classes: ["btn", "btn-danger", "btn-sm"], attrs: { onclick: `deleteTool('${tool.id}')` } });
    ...
    function kv(k, v, color) {
        ...
        row.append(el("span", { text: k, attrs: { style: "color:var(--muted);" } }), el("span", { text: v, attrs: { style: color ? `color:${color};font-weight:700;` : "" } }));
        return row;
    }
}
window.showAddToolModal = function () {
    const form = el("form", { classes: ["form"], attrs: { onsubmit: "handleAddTool(event)" } });
    form.append(el("input", { attrs: { type: "text", name: "name", placeholder: "Nome da ferramenta", required: "true" } }), el("textarea", { attrs: { name: "description", placeholder: "Descrição (opcional)", rows: "3" } }), el("input", { attrs: { type: "number", name: "quantity", placeholder: "Quantidade atual", min: "0", required: "true" } }), el("input", { attrs: { type: "number", name: "min_quantity", placeholder: "Quantidade mínima", min: "0", required: "true" } }), el("input", { attrs: { type: "text", name: "location", placeholder: "Localização (opcional)" } }), btnRow());
    ...
    function btnRow() {
        ...
        const cancel = el("button", { classes: ["btn", "btn-secondary"], attrs: { type: "button", onclick: "closeModal()" } });
...
window.handleAddTool = async function (event) {
    ...
    if (error)
        return showToast("Erro ao adicionar ferramenta!", "error");
    window.closeModal();
    await fetchTools();
...
window.editTool = function (id) {
    ...
    window.handleEditTool = async function (event) {
        ...
        if (error)
            return showToast("Erro ao atualizar ferramenta!", "error");
        window.closeModal();
        await fetchTools();
...
window.deleteTool = function (id) {
    ...
    box.append(el("p", { text: `Tem certeza que deseja excluir a ferramenta "${t.name}"?`, attrs: { style: "font-size:1.05rem;margin-bottom:18px;" } }), el("p", { text: "Esta ação não pode ser desfeita.", attrs: { style: "color:var(--muted);margin-bottom:26px;" } }));
    const row = el("div", { attrs: { style: "display:flex;gap:12px;justify-content:center;" } });
    const ok = el("button", { classes: ["btn", "btn-danger"], attrs: { onclick: `confirmDelete('${id}')` } });
    ok.innerHTML = '<i class="fas fa-trash"></i> Excluir';
    const cancel = el("button", { classes: ["btn", "btn-secondary"], attrs: { onclick: "closeModal()" } });
    cancel.innerHTML = '<i class="fas fa-times"></i> Cancelar';
    ...
    window.confirmDelete = async function (toolId) {
        ...
        if (error)
            return showToast("Erro ao excluir ferramenta!", "error");
        window.closeModal();
        await fetchTools();
...
window.showDetailedReport = function () {
    ...
    c.append(el("p", { text: `Gerado em: ${new Date().toLocaleDateString("pt-BR")} às ${new Date().toLocaleTimeString("pt-BR")}`, attrs: { style: "color:var(--muted);font-size:.9rem;margin-bottom:20px;" } }));
    ...
    grid.append(stat("Total de Ferramentas", totalTools), stat("Estoque Baixo", low), stat("Total de Itens", totalQty), stat("Média por Item", avg));
    c.append(grid);
    ...
    if (!lows.length) {
        ...
        lows.forEach((t) => {
            list.append(el("div", { attrs: { style: "padding:12px;border:1px solid var(--warning);border-radius:12px;margin:8px 0;background:rgba(255,193,7,.1);" } })
                .appendChild(el("span", { text: `${t.name} — ${t.quantity}/${t.min_quantity} (${t.location || "Sem local"})` })) && list.lastChild);
        });
    }
    sec.append(list);
    c.append(sec);
    ...
    function stat(label, value) {
        ...
        k.append(el("h4", { text: label, attrs: { style: "margin:0 0 8px 0;font-size:1rem;color:var(--sec2);" } }), el("p", { text: String(value), attrs: { style: "font-size:2em;color:var(--sec2);font-weight:bold;margin:0;" } }));
        return k;
    }
};
