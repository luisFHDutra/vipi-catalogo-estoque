...
async function renderList() {
    ...
    const btnClear = el("button", { classes: ["btn", "ghost"], text: "Limpar" });
    ...
    const btnPrev = el("button", { classes: ["btn", "ghost"], text: "‹ Anterior" });
    const btnNext = el("button", { classes: ["btn", "ghost"], text: "Próxima ›" });
...
function renderForm(existing) {
    ...
    if (existing) {
        ...
        if (Array.isArray(existing.images))
            currentImages = existing.images.slice();
    }
    refreshThumbs();
    form.addEventListener("submit", async (ev) => {
        ...
        const files = document.getElementById("fImgs")?.files;
