...
function html(str: string) { const t = document.createElement("template"); t.innerHTML = str.trim(); return t.content.firstElementChild as HTMLElement; }
...
function ensureHeader(titleText: string) {
    ...
    `);
        ...
        if (brand) {
            ...
            header.appendChild(html(`<div class="brand"><img class="logo" src="./assets/logo-vipimatrizes.jpg" alt="VIPI Matrizes"/><strong>${titleText}</strong></div>`));
        }
...
function autoTitle() {
    ...
    if (p.toLowerCase().includes("estoque")) return "Estoque de Ferramentas";
